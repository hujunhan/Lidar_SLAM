...
@ti.kernel
def point_based_matching_taichi(point_pairs:ti.types.ndarray(),output:ti.types.ndarray(),n:ti.i32):
    x_mean = 0.0
    y_mean = 0.0
    xp_mean = 0.0
    yp_mean = 0.0
    # n = tm.length(point_pairs)
    ...
    for i in range(n):
        x = point_pairs[i,0,0]
        y=point_pairs[i,0,1]
        xp=point_pairs[i,1,0]
        yp=point_pairs[i,1,1]
    ...
    rot_angle = tm.atan2(s_x_yp - s_y_xp, s_x_xp + s_y_yp)
    translation_x = xp_mean - (x_mean*tm.cos(rot_angle) - y_mean*tm.sin(rot_angle))
    translation_y = yp_mean - (x_mean*tm.sin(rot_angle) + y_mean*tm.cos(rot_angle))
    output[0]=rot_angle
    output[1]=translation_x
    output[2]=translation_y
...
def icp(reference_points, points, max_iterations=100, distance_threshold=0.3, convergence_translation_threshold=1e-3,
    ...
    for iter_num in range(max_iterations):
        ...
        closest_point_pairs=np.asarray(closest_point_pairs,dtype=np.float32)
        
        point_based_matching_taichi(closest_point_pairs,output,len(closest_point_pairs))
        closest_rot_angle=output[0]
        closest_translation_x=output[1]
        closest_translation_y=output[2]
        # print(f'shape of closest_point_pairs is {np.asarray(closest_point_pairs).shape}')
